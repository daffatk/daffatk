<h1>Peta Sebaran Pos Curah Hujan</h1>
<div class="content"> 
...
var kab = new L.LayerGroup();
var curah = new L.LayerGroup();

var map = L.map('map', { 
 center: [-1.126460837954284, 120.19060322600956],
 zoom: 5, 
 zoomControl: false,
 layers:[] 
}); 
...
maxZoom: 22, 
attribution: 'Daffa Rizal' }).addTo(map);
...
$.getJSON("<?=base_url()?>assets/Curah_Hujan.geojson",function(data){
var ratIcon = L.icon({
...
var marker = L.marker(latlng,{icon: ratIcon});
marker.bindPopup(feature.properties.nm_balai);
return marker;
}
}).addTo(curah);
});
...
$.getJSON("<?=base_url()?>/assets/provinsi_poligon.geojson",function(kode){
 ...
 $.getJSON("<?=base_url()?>/assets/kabupaten.geojson",function(kode){
 L.geoJson( kode, {
 style: function(feature){
 var fillColor,
 kode = feature.properties.kode;
 ...
 onEachFeature: function( feature, layer ){
 layer.bindPopup(feature.properties.KAB)
 }
 }).addTo(kab);
 });
